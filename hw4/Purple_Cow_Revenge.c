...
bool Union(int shop1, int shop2, DisjointSet **arr_disjoint, Insert **arr_insert, int current_day)
{
    ...
    if (arr_disjoint[root1]->rank > arr_disjoint[root2]->rank)
    { // 1 > 2
    ...
    else if (arr_disjoint[root1]->rank < arr_disjoint[root2]->rank)
    { // 1 < 2
...
int main(void)
{
    ...
    for (int i = 0; i <= M; i++)
    {
        if (arr_input[i]->instruction == 'b')
        { // boom
            current_day = i;
            back_day = arr_input[i]->first;
            while (arr_input[back_day]->instruction == 'b')
            { // still boom
