...
// ""
// 	"
// 	This homework returns None
// 		Score on Judge : 5(return 0) ""
// 									 "
...
Node *Merge(Node *left, Node *right)
{
	...
	if (right->random < left->random)
	{ // 根據上面Split: a一定在b的左邊, 而若a的random(priority)>b的, 代表b一定為a的right child
		left->right = Merge(left->right, right);
		UpdateUp(left);
		return left;
	}
	else
	{ // 根據上面Split: a一定在b的左邊, 而若a的random(priority)<b的, 代表a一定為b的left child
